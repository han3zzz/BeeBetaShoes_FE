window.HomeController = function($http,$scope,$routeParams)
{
...
$('#rangeMaxTL').on('input', function(e) {
	...
	$scope.detail = function(){
		...
		$http.get("http://localhost:8080/api/product/"+id).then(function(response){
			...
			$http.get("http://localhost:8080/api/product/category/"+response.data.category.id).then(function(e){
		...
		$http.get("http://localhost:8080/api/color/get/"+id).then(function(color){
		...
		$scope.addToCart = function(){
			...
			if($scope.quantity < soLuong){
				Swal.fire("Số lượng bạn thêm đang vượt quá số lượng còn hàng !!","","error");
				return;
			}
			...
			$http.get("http://localhost:8080/api/product/"+id).then(function(response){
				...
				if(response.data.discount > 0){
					unitPrice = response.data.price - (response.data.price * (response.data.discount * 0.01))
			...
			// add to cart
				...
				$http.post("http://localhost:8080/api/cart",{
					idCart : 1 ,
					idProductDetail : id,
					idColor : selectedVal,
					idSize : selectedVal1,
			...
			if($scope.listCart.length > 0){
				for(let i = 0; i < $scope.listCart.length; i++){
					// if product in cart
					if($scope.listCart[i].productDetail.id == id && $scope.listCart[i].idColor == selectedVal && $scope.listCart[i].idSize == selectedVal1){
					
						if($scope.listCart[i].quantity  >= $scope.quantity){
							Swal.fire("Bạn đã thêm số lượng tối đa hiện có của sản phẩm vào giỏ hàng !!","","error");
							return;
						}
						
						$http.put("http://localhost:8080/api/cart/updateCart/"+$scope.listCart[i].id,{

							idCart : 1 ,
							idProductDetail : id,
							idColor : selectedVal,
							idSize : selectedVal1,
							quantity : parseInt(soLuong) + parseInt($scope.listCart[i].quantity),
							unitPrice : unitPrice
	...
	$scope.loadCart = function(){
		...
		$http.get("http://localhost:8080/api/cart/1").then(function(cart){
			...
			$scope.tongTien = 0 ;
		for(let i = 0; i < $scope.listCart.length; i++){
			$scope.tongTien += $scope.listCart[i].unitPrice * $scope.listCart[i].quantity;
		}
		});
		...
		$scope.deleteByCart = function(id){
			...
			$scope.giam = function(idCartDetail,idProductDetail,idColor,idSize){
				var getQuanity = parseInt(document.getElementById("qty"+idCartDetail).value);
				...
				$http.get("http://localhost:8080/api/product/"+idProductDetail).then(function(response){
					var unitPrice = 0 ;
					if(response.data.discount > 0){
						unitPrice = response.data.price - (response.data.price * (response.data.discount * 0.01))
				...
				$http.put("http://localhost:8080/api/cart/updateCart/"+idCartDetail,{
			...
			$scope.tang = function(idCartDetail,idProductDetail,idColor,idSize){
					...
					$http({
						...
						if(document.getElementById("qty"+idCartDetail).value >= $scope.quantity){
							Swal.fire("Số lượng đã đến mức tối đa số lượng sản phẩm hiện có !","","error");
							return;
						}
						document.getElementById("qty"+idCartDetail).value = parseInt(document.getElementById("qty"+idCartDetail).value) + 1 ;
				...
				idColor : idColor,
				idSize : idSize,
				quantity : parseInt(document.getElementById("qty"+idCartDetail).value),
				unitPrice : unitPrice
			...
			$scope.EnterQuantity = function(idCartDetail,idProductDetail,idColor,idSize){
				
				var numberRegex = /^[0-9]+$/;
				if(!numberRegex.test(document.getElementById("qty"+idCartDetail).value)){
					Swal.fire("Số lượng phải là số nguyên dương !!","","error");
					$http.get("http://localhost:8080/api/cart/getQuantityByCartDetail/"+idCartDetail).then(function(resp){
	...
	/*-----------------------------------------
	 ...
	 /*-----------------------------------------
	  ...
	  function login_dropdown(){
		...
		$("body").on("click", function(event ) {
		  var $target = $(event.target);
		  if(!$target.parents().is(".site-login") && !$target.is(".site-login")){
