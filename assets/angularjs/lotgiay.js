...
window.LotGiayController = function($scope, $http, $location,$routeParams){
    let url = "http://localhost:8080/api/heelcushion";
    ...
    $scope.add = function(){
        $http.post(url,{
            ...
            if(resp.status === 200){
                ...
                setTimeout(() => {
                    location.href = "#/heelcushions/view";
                }, 2000);
            }
        }).catch(function (err){
            if (err.status === 400){
      ...
      $scope.update = function(){
        ...
        $http.put("http://localhost:8080/api/heelcushion/update/"+id,{
            name : $scope.form.name,
            description : $scope.form.description
        }).then(function(resp){
            if(resp.status === 200){
      ...
      $scope.delete = function (id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $http.put("http://localhost:8080/api/heelcushion/delete/"+id).then(function (response){
    ...
    $scope.detail = function(){
        let id = $routeParams.id ;
        $http.get("http://localhost:8080/api/heelcushion/" + id).then(function(resp){
    ...
    $scope.search = function (){
        ...
        if (name.trim().length === 0){
            ...
            $http.get("http://localhost:8080/api/heelcushion/search/"+name).then(function (search){
